...
class PySelenium(object):
    ...
    def __init__(self, browser='ff',driver=None, remoteAddress=None):
        """
        remote consle：
        ...
        if remoteAddress is None:
            if browser == "firefox" or browser == "ff":
                dr = webdriver.Firefox(executable_path=driver)
            elif browser == "chrome" or browser == "Chrome":
                dr = webdriver.Chrome(executable_path=driver)
            elif browser == "internet explorer" or browser == "ie":
                dr = webdriver.Ie(executable_path=driver)
            elif browser == "opera":
                dr = webdriver.Opera(executable_path=driver)
            elif browser == "phantomjs":
                dr = webdriver.PhantomJS(executable_path=driver)
            elif browser == "edge":
                dr = webdriver.Edge(executable_path=driver)
            # elif browser == "360se":
            #     __browser_url = r'C:\Users\dell\AppData\Roaming\360se6\Application\360se.exe' #360浏览器的地址,未成功
            #     chrome_options = Options()
            #     chrome_options.binary_location = __browser_url
            #     dr = webdriver.Chrome(executable_path=driver,chrome_options = chrome_options)
        else:
            if browser == "RChrome":
    ...
    def element_is_not_visible(self,css,secs=5):
        """
        Waiting for an element to invisible.

        Usage:
        driver.element_is_not_visible("id->kw")
        """
        if "->" not in css:
            raise NameError("Positioning syntax errors, lack of '->'.")
        ...
        if by == "id":
            WebDriverWait(self.driver, secs, 1).until(EC.invisibility_of_element_located((By.ID, value)), messages)
        elif by == "name":
            WebDriverWait(self.driver, secs, 1).until(EC.invisibility_of_element_located((By.NAME, value)), messages)
        elif by == "class":
            WebDriverWait(self.driver, secs, 1).until(EC.invisibility_of_element_located((By.CLASS_NAME, value)),
                                                          messages)
        elif by == "link_text":
            WebDriverWait(self.driver, secs, 1).until(EC.invisibility_of_element_located((By.LINK_TEXT, value)),
                                                          messages)
        elif by == "xpath":
            WebDriverWait(self.driver, secs, 1).until(EC.invisibility_of_element_located((By.XPATH, value)), messages)
        elif by == "css":
            WebDriverWait(self.driver, secs, 1).until(EC.invisibility_of_element_located((By.CSS_SELECTOR, value)),
                                                          messages)
        else:
            raise NameError(
                    "Please enter the correct targeting elements,'id','name','class','link_text','xpaht','css'.")

    def element_is_visible(self,css,secs=5):
        """
        Waiting for an element to visible.

        Usage:
        driver.element_is_visible("id->kw")
        """
        if "->" not in css:
            raise NameError("Positioning syntax errors, lack of '->'.")
    ...
    def fullscreen_window(self):
        ...
        self.my_print("{0} Set browser window fullscreen, Spend {1} seconds".format(success, time.time() - t1))
    ...
    def clear_type(self, css, text):
        ...
        try:
            ...
            actions.perform()
            # el.send_keys(text)
            self.my_print("{0} Clear and typed element: <{1}> content: {2}, Spend {3} seconds".format(success,
                                                                                            css, text,
                                                                                            time.time() - t1))
        except Exception:
    ...
    def click(self, css, secs = 5):
        ...
        try:
            self.element_wait(css)
            if by == "id":
                WebDriverWait(self.driver, secs, 1).until(EC.element_to_be_clickable((By.ID, value)), messages).click()
            elif by == "name":
                WebDriverWait(self.driver, secs, 1).until(EC.element_to_be_clickable((By.NAME, value)), messages).click()
            elif by == "class":
                WebDriverWait(self.driver, secs, 1).until(EC.element_to_be_clickable((By.CLASS_NAME, value)), messages).click()
            elif by == "link_text":
                WebDriverWait(self.driver, secs, 1).until(EC.element_to_be_clickable((By.LINK_TEXT, value)), messages).click()
            elif by == "xpath":
                WebDriverWait(self.driver, secs, 1).until(EC.element_to_be_clickable((By.XPATH, value)), messages).click()
            elif by == "css":
                WebDriverWait(self.driver, secs, 1).until(EC.element_to_be_clickable((By.CSS_SELECTOR, value)),messages).click()
            else:
                raise NameError("Please enter the correct targeting elements,'id','name','class','link_text','xpaht','css'.")
    ...
    def retry_find_click(self,css,secs = 5):
        t1 = time.time()
        result = False
        attempts = 0
        while attempts < 2:
            try:
                ...
                self.my_print(
                    "{0} retry find click element: <{1}>, Spend {2} seconds".format(success, css, time.time() - t1))
            attempts += 1
        return result

    def move_to_click(self, css, secs = 5):
        """
        Mouse over the element to click.

        Usage:
        driver.move_to_click("id->kw")
        """
        t1 = time.time()
        try:
            ...
            self.element_is_visible(css,secs)
            self.click(css, secs)
            self.my_print("{0} Move to element and click: <{1}>, Spend {2} seconds".format(success, css, time.time() - t1))
        except Exception:
            self.my_print("{0} unable move to element and click: <{1}>, Spend {2} seconds".format(fail, css, time.time() - t1))
            raise
    ...
    def select(self,css,option):
        ...
        if by == "index":
            ...
            Select(ele).all_selected_options()
        else:
            raise NameError(
                "Please enter the correct targeting options,'index','value','text','all'")
    ...
    def is_displayed(self,css):
        ...
        try:
            ...
            el = self.get_element(css)
            el.is_displayed()
            self.my_print("{0} Typed element: <{1}> , Spend {2} seconds".format(success,css,time.time() - t1))
        except Exception:
            self.my_print("{0} Unable to type element: <{1}> , Spend {2} seconds".format(fail,css,time.time() - t1))
            raise
